import { Avatar, HStack, VStack, Text, Icon, theme } from '@holdr-ui/react';
...
function MembershipItem({ data }: MembershipItemProps) {
  return (
    ...
    >
      ...
      <VStack gap={2}>
        <TextGroup gap={0}>
          <TextGroupHeading size={2}css={{lineHeight: 1.45}}>{data.name}</TextGroupHeading>
          <TextGroupSubheading size={1} weight={300} color='white700' css={{lineHeight: 1}}>
            {`Membership #${data.membershipNum}`}
          </TextGroupSubheading>
        </TextGroup>
        <HStack gap={1} items='center'>
          <Icon name={data.priceHasRisen ? 'arrow-up-outline' : 'arrow-down-outline'} color={data.priceHasRisen ? 'success500' : 'danger400'} />
          <Text weight={500} size={2} color={data.priceHasRisen ? 'success500' : 'danger400'}>
